...
#YOU DO NEED TO HAVE MYSQL INSTALLED!!!! also you need python-mysqldb installed (sudo apt-get install)
#connect is used:(location of db, username, password, the database name)
con = mdb.connect('localhost', 'odin', 'odintest123', 'odindatabase')
testXml = '<?xml version="1.0" encoding="ASCII"?>' #<?xml version="1.0" encoding="ASCII"?><data><ticker symbol="FB" market="DJI"><s>FB</s><n0>Facebook, Inc.</n0><o0>79.37</o0><l1>80.04</l1><p0>78.37</p0></ticker><ticker symbol="IBM" market="DJI"><n0>International Bus</n0><s0>IBM</s0><l1>162.18</l1><o0>162.11</o0><p0>161.79</p0></ticker><ticker symbol="GOOG" market="IXIC"><n0>Google Inc.</n0><l1>543.98</l1><s0>GOOG</s0><o0>539.32</o0><p0>532.71</p0></ticker><ticker symbol="^XAU" market="DJI"><n0>PHLX Gold/Silver </n0><s0>^XAU</s0><l1>76.25</l1><o0>75.656</o0><p0>75.927</p0></ticker></data>'
#DEBUG
...
def ParseXml():
  ...
  t = str(time.time()) #useful for noting what time we got out update (in UTC)
...
def ParseYggdrasilXml(_yggdrasilXml):
  ...
  for child in tree:
    ...
    globalTicker = child.get("symbol")
    ticker = child.get("symbol") # Gets symbol e.g FB
    # market = child.get("market") # Gets market e.g DJI
    fields.append(('s0', ticker))
...
def AddTickerData(_fields):
  ...
  ToExecute = "UPDATE StockData SET "
  flag = True
  for field in _fields:
    if field[0] == "s0":
      ticker = field[1]
    ToExecute += field[0] #BUILD THE QUERY!!!!! yay
    ToExecute += "="
    ToExecute += "'" + field[1] + "',"

  ToExecute = ToExecute[:-1]
  ToExecute += " WHERE s0='" + ticker + "';"

  print ToExecute #DEBUG
...
def PrintData():
  with con:
    cur = con.cursor()
    cur.execute("SELECT * FROM StockData") #query
...
# ResetData()
#ParseXml()
while 1: #keep running in the background and log the updates
  toStore = ""
  f = open("../yggdrasil/bifrost") #read from bifrost
  toStore = testXml + f.read()
  f.close()
  ParseYggdrasilXml(toStore)
